...
"""Module for generating and printing directory structures."""
...
def print_dir_tree(startpath, ignore_clutter=False):
    ...
    for root, dirs, files in os.walk(startpath, topdown=True):
        ...
        dirs[:] = [
            ...
            if not (ignore_clutter and should_ignore(os.path.join(root, d), ignored_patterns))
        ]
        files[:] = [
            f
            for f in sorted(files)
            if not (ignore_clutter and should_ignore(os.path.join(root, f), ignored_patterns))
        ]
    ...
    files_in_root = sorted(
        [
            ...
            and not (ignore_clutter and should_ignore(os.path.join(startpath, f), ignored_patterns))
        ]
    )
