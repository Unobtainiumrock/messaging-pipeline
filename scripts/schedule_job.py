...
def schedule_job(frequency: str = 'hourly') -> bool:
    ...
    try:
        ...
        if frequency == 'hourly':
            ...
            logger.info(f"Setting custom schedule: {args.minute} {args.hour} {args.day} {args.month} {args.weekday}")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('action', choices=['schedule', 'remove', 'display'], 
                      help='Action to perform (schedule, remove, or display jobs)')
    parser.add_argument('--frequency', choices=['hourly', 'daily', 'custom'], default='hourly',
